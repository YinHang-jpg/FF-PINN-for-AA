...
def main():
    ...
    Y_theory = theoretical_Vt_PP(R_raw, Theta_raw).detach()
    ...
    model = WakeNetVtPP(n_harmonics=8).to(device)
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Training Loss - Wake Vt_PP')
    plt.grid(True)
    plt.show()
    ...
    with torch.no_grad():
        ...
        true_val = theoretical_Vt_PP(RR_flat, TT_flat)
    ...
    torch.save(model.state_dict(), os.path.join(save_dir, 'wake_Vt_PP.pth'))
    with open(os.path.join(save_dir, 'wake_Vt_PP_norm.json'), 'w') as f:
        json.dump({
