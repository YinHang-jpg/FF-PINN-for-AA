...
class WakeNetVrPP(nn.Module):
    ...
    def forward(self, R_scaled, Theta):
        phases = self.harmonics * Theta  # (N, n_harmonics)
...
def theoretical_Vr_PP(R, Theta, Re=Re_FIXED):
    cos_t = torch.cos(Theta)
    cos_2t = torch.cos(2 * Theta)
    R2 = R ** 2
    R4 = R ** 4
    term1 = 4 * R * (16 + 3 * Re + 3 * R2 * (-16 + (2 * R - 3) * Re)) * cos_t
    term2 = 3 * ((R - 1) ** 2) * (1 + R + 2 * R2) * (1 + 3 * cos_2t) * Re
    return (term1 - term2) / (128 * R4)
...
def main():
    ...
    print("PINN 训练: Wake PP 径向速度 Vr_PP(R, Theta)")
    ...
    Y_theory = theoretical_Vr_PP(R_raw, Theta_raw).detach()
    ...
    model = WakeNetVrPP(n_harmonics=8).to(device)
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Training Loss - Wake Vr_PP')
    plt.grid(True)
    plt.show()
    ...
    with torch.no_grad():
        ...
        true_val = theoretical_Vr_PP(RR_flat, TT_flat)
    ...
    torch.save(model.state_dict(), os.path.join(save_dir, 'wake_Vr_PP.pth'))
    with open(os.path.join(save_dir, 'wake_Vr_PP_norm.json'), 'w') as f:
        json.dump({
