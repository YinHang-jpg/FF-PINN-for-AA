...
from wake_theory_torch import dimless_vt_oseen as _dimless_vt_oseen  # noqa: E402
from wake_pinn_train_config import R_OSEEN_R_MAX, R_OSEEN_R_MIN  # noqa: E402

theoretical_Vt_Oseen = partial(_dimless_vt_oseen, Re=Re_FIXED)
...
def main():
    ...
    print("PINN 训练: Wake Oseen 切向速度 Vt_Oseen(R, Theta)")
    print(f"device: {device}, Re = {Re_FIXED}")

    R_min, R_max = R_OSEEN_R_MIN, R_OSEEN_R_MAX
    ...
    Y_theory = theoretical_Vt_Oseen(R_raw, Theta_raw).detach()
    ...
    model = WakeNetVtOseen(n_harmonics=16).to(device)
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Training Loss - Wake Vt_Oseen')
    plt.grid(True)
    plt.show()
    ...
    with torch.no_grad():
        ...
        true_val = theoretical_Vt_Oseen(RR_flat, TT_flat)
    ...
    torch.save(model.state_dict(), os.path.join(save_dir, 'wake_Vt_Oseen.pth'))
    with open(os.path.join(save_dir, 'wake_Vt_Oseen_norm.json'), 'w') as f:
        json.dump({
